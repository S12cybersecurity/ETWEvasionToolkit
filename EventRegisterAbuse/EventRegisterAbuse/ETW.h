...
public:
    ...
    void BroadcastEvent(const std::wstring& message) {
        ...
        // 1) Descriptor de evento VÁLIDO (nunca nullptr)
        ...
        // 2) Datos del evento (mensaje wide, tamaño en BYTES)
        EVENT_DATA_DESCRIPTOR eventData[1];
        const ULONG msgBytes = static_cast<ULONG>((message.size() + 1) * sizeof(wchar_t));
        EventDataDescCreate(&eventData[0], message.c_str(), msgBytes);

        // 3) Emitir a cada proveedor registrado
        int successCount = 0;
        for (REGHANDLE handle : registeredHandles) {
            ULONG result = EventWrite(handle, &desc, _countof(eventData), eventData);
            if (result == ERROR_SUCCESS) {
                successCount++;
            }
            else {
                // (opcional) loguea el error para diagnosticar casos individuales
                // printf("EventWrite failed for handle %p: %lu\n", (void*)handle, result);
            }
        }
